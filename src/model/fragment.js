...
class Fragment {
  ...
  static async put(ownerId, id, data, type) {
    try {
      ...
      const existingFragment = await Fragment.byId(ownerId, id); // 2. Validate the new content type
      ...
      logger.error({ err }, 'Failed to put fragment'); // Re-throw the error so the calling function can handle it
      throw err;
    }
  }
}
